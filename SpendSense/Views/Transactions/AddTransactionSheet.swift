                ...
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 24) {
                                ...
                                Button(action: {
                                    ...
                                    VStack(spacing: 6) {
                                        Text(mode.rawValue)
                                            ...
                                            .foregroundColor(addVM.mode == mode ? .ssAccent : .ssTextSecondary)
                                        Rectangle()
                                            .fill(addVM.mode == mode ? Color.ssAccent : Color.clear)
                                            .frame(height: 2)
                                    }
                                }
                        ...
                        VStack(spacing: 8) {
                            ...
                            Divider()
                                .background(amountFocused ? Color.ssAccent.opacity(0.6) : Color.ssBorder)
                                .padding(.horizontal, 24)
                        ...
                        // Wishlist name & savings plan (only for wishlist mode)
                        if addVM.mode == .wishlist {
                            VStack(alignment: .leading, spacing: 8) {
                                Text("Item Name")
                                    .font(SSFont.body(12))
                                        ...
                                        HStack {
                                            VStack(alignment: .leading, spacing: 2) {
                                                ...
                                                Text("Rs. \(Int(addVM.dailySavingsPreview))")
                                                    .font(SSFont.mono(16, weight: .bold))
                                                    .foregroundColor(.ssAccent)
                                            }
                                            Spacer()
                                            VStack(alignment: .trailing, spacing: 2) {
                                                Text("Target date")
                                                    .font(SSFont.body(11))
                                                    .foregroundColor(.ssTextTertiary)
                                                Text(Calendar.current.date(byAdding: .day, value: addVM.savingsDays, to: Date())?.formatted(.dateTime.day().month(.abbreviated)) ?? "--")
                                ...
                                HStack(spacing: 6) {
                                    ...
                                    Text("Rs. \(Int(addVM.dailySavingsPreview))/day will be deducted from your budget for \(addVM.savingsDays) days")
                                        .font(SSFont.body(12))
                                        .foregroundColor(.ssTextSecondary)
                                }
                                .padding(10)
                        ...
                        // Action button
                        VStack(spacing: 12) {
                            if addVM.mode == .simulate && !addVM.showSimulation {
                                Button(action: { addVM.runSimulation(using: vm) }) {
                                    ...
                                    .background(
                                        Group {
                                            if addVM.isValid {
                                                LinearGradient.ssAccentGradient.cornerRadius(14)
                                            } else {
                                                Color.ssBorder.cornerRadius(14)
                                            }
                                        }
                                    )
                                }
                                .disabled(!addVM.isValid)
                            } else if addVM.mode == .simulate && addVM.showSimulation {
                                Button(action: {
                                    addVM.confirm(using: vm)
                                    dismiss()
                                }) {
                                    HStack(spacing: 8) {
                            ...
                            } else if addVM.mode == .wishlist {
                                Button(action: {
                                    addVM.addToWishlist(using: vm)
                                    dismiss()
                                }) {
                                    HStack(spacing: 8) {
                                    ...
                                    .frame(maxWidth: .infinity)
                                    .frame(height: 54)
                                    .background(LinearGradient(colors: [.ssViolet, Color(hex: "#A855F7")], startPoint: .leading, endPoint: .trailing).cornerRadius(14))
                                }
...
struct CategoryChip: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .background(
                isSelected
                    ? category.color.cornerRadius(20)
                    : Color.ssSurfaceElevated.cornerRadius(20)
            )
            .overlay(
                Capsule()
                    .stroke(isSelected ? category.color : Color.ssBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
        ...
        VStack(spacing: 16) {
            HStack {
                ...
                Text("Purchase Impact Preview")
                    ...
                    .foregroundColor(.ssTextPrimary)
                Spacer()
                Image(systemName: result.riskLevelAfter.icon)
                    .foregroundColor(result.riskLevelAfter.color)
            }
            ...
            VStack(spacing: 12) {
                ...
                SimRow(label: "Budget Remaining After", value: vm.formatCurrency(max(0, result.remainingAfter)),
                       color: result.riskLevelAfter.color)
                SimRow(label: "Budget Used After", value: "\(Int(result.progressAfter * 100))%",
                       color: result.riskLevelAfter.color)
                SimRow(label: "Risk Level", value: result.riskLevelAfter.rawValue,
                       color: result.riskLevelAfter.color)
            }

            // Progress bar after purchase
            VStack(alignment: .leading, spacing: 6) {
                Text("Budget after purchase")
                    .font(SSFont.body(11))
                    .foregroundColor(.ssTextTertiary)
            ...
            if let warning = result.categoryWarning {
                ...
                .padding(10)
                .background(Color.ssWarning.opacity(0.1))
                .cornerRadius(8)
            }
            ...
            if !result.isSafe {
                ...
                .padding(10)
                .background(Color.ssDanger.opacity(0.1))
                .cornerRadius(8)
            }
        }
        .padding(16)
